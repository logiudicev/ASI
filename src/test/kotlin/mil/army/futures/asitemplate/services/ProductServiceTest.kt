...
internal class ProductServiceTest {
    ...
    fun `should update a product`() {
        ...
        every { productRepository.findByIdOrNull(1L) } returns existingProduct
        every { productRepository.save(any()) } answers { firstArg() }

        val productToSave = Product(id = 1L, name = "Wrench", quantity = 6)

        productService.updateProduct(1, productToSave)
