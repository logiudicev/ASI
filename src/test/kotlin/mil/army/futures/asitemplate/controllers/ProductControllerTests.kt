...
internal class ProductControllerTests {
    ...
    @Test
    fun `should update a product when product is updated`() {
        every { productService.updateProduct(any(), any()) } returns Unit

        mockMvc.post("/products/1") {
            contentType = MediaType.APPLICATION_JSON
            content = "{ \"id\": \"1\", \"name\": \"wrench\", \"quantity\": \"3\" }"
        }.andExpect {
            status { isOk() }

        }

        val productToBeUpdated = Product(id = 1L, name = "wrench", quantity = 3)

        verify(exactly = 1) {
            productService.updateProduct(1L, productToBeUpdated)
        }
    }
    }
